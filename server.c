...
void *move_jogador( void *dados){
	...
	do{
		...
		if(pos_ocupadas[j->num_oc] == 0){
			...
			switch(ch){
				case 0:
					if(init_xy_Jogador[pos_ocupadas[j->num_oc]][1]!=0)
						init_xy_Jogador[j->num_oc][1]--;
				case 1:
				if(init_xy_Jogador[pos_ocupadas[j->num_oc]][1]!=20)
					init_xy_Jogador[j->num_oc][1]++;
				case 2:
				if(init_xy_Jogador[pos_ocupadas[j->num_oc]][0]!=0)
					init_xy_Jogador[j->num_oc][0]--;
				case 3:
				if(init_xy_Jogador[pos_ocupadas[j->num_oc]][0]!=50)
					init_xy_Jogador[j->num_oc][0]++;
			}
			...
			if(isStarted==0){
				...
				for(i=0;i<num_jog;i++){
					...
					if((i>0&&i<=defesas)||(i>num_jog/2&&i<=num_jog/2+defesas))
						...
						*/
					if(pthread_create(&tarefa[i],NULL,&move_jogador,(void *)&jog[i])!=0){
						printf("ERRO-thread");
					}
					else{
						//printf("Criou %d",i);
